...
export class VehicleDb {
    ...
    public addAll(locations: IVehicleLocationExtended[]): void {
        const dataMap: VehicleIdMap = locations.reduce<VehicleIdMap>((prev: VehicleIdMap, cur: IVehicleLocationExtended): VehicleIdMap => {
            if (prev.has(cur.id)) {
        ...
        this.mLastUpdate = this.mVehicles
            .reduce((prev: number, cur: IVehicleLocationExtended): number => {
                return Math.max(prev, cur.lastUpdate);
            }, 0);
     ...
     * @since 3.0.0 
     */
    public getVehiclesIn(left: number, right: number, top: number, bottom: number, since: number = 0): IVehicleLocationExtended[] {
        if (left >= right) {
